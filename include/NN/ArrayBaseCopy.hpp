...
namespace MX
{
    ...
    template<typename T>
    Array<T> Random(const Array<typename Array<T>::size_type> &shape, double from=0.0, double to=1.0);

    template<typename T>
    Array<T> Sequence(const Array<typename Array<T>::size_type> &shape, double start=0.0, double step=1.0);

    template<typename T>
    Array<T> Zeros(const Array<typename Array<T>::size_type> &shape);

    template<typename T>
    Array<T> Ones(const Array<typename Array<T>::size_type> &shape);
...
namespace MX
{
    ...
    public:
        ...
        friend Array Random<>(const Array<size_type> &shape, double from, double to);
        friend Array Sequence<>(const Array<size_type> &shape, double start, double step);
        friend Array Zeros<>(const Array<size_type> &shape);
        friend Array Ones<>(const Array<size_type> &shape);
        ...
        friend Array Transpose<>(const Array &array, const Array<depth_type>& order);
