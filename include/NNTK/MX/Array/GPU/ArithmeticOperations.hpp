...
#define MX_ARRAY_OPERATION_AA_REV_GPU(op, name, check)                        \
  template<typename T, Device D, Device V>                                    \
    void                                                                      \
    name##_array_array_reverse_gpu(Array<T, D>& result,                       \
                                   const Array<T, V>& array)                  \
    {                                                                         \
      if (check)                                                              \
        for (auto& e : array)                                                 \
          NN_RUNTIME_ERROR(e == 0, "array: division by zero")                 \
      name##_array_array_reverse_cuda(result.data(), array.data(),            \
                                      result.size());                         \
    }

  MX_ARRAY_OPERATION_AA_REV_GPU(-, subtract, 0)
  MX_ARRAY_OPERATION_AA_REV_GPU(/, divide, 1)
