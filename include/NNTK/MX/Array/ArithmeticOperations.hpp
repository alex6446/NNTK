...
#define MX_ARRAY_OPERATION_AA_REV_CPU(op, name, check)                        \
  template<typename T, Device D, Device V>                                    \
    void                                                                      \
    name##_array_array_reverse_cpu(Array<T, D>& result,                       \
                                   const Array<T, V>& array)                  \
    {                                                                         \
      for (T *resd = result.data(), *arrd = (T*) array.data();                \
           resd != result.data() + result.size(); ++resd, ++arrd) {           \
        if (check) NN_RUNTIME_ERROR(!(*resd), "array: division by zero")      \
        *resd = *arrd op *resd;                                               \
      }                                                                       \
    }

  MX_ARRAY_OPERATION_AA_REV_CPU(-, subtract, 0)
  MX_ARRAY_OPERATION_AA_REV_CPU(/, divide, 1)
