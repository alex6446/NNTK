...
namespace NN {
    ...
    namespace MX {
        ...
        public: // friends go here
            ...
            friend Matrix operator+ (T a, const Matrix& B) {
                if (!B.arr)
                    throw std::runtime_error(":operator+:1: matrix not initialized");
                Matrix C = B;
                for (int i = 0; i < C.m; ++i)
                    for (int j = 0; j < C.n; ++j)
                        C.arr[i][j] += a;
            ...
            friend Matrix operator/ (T a, const Matrix& B) {
                ...
                for (int i = 0; i < C.m; ++i)
                    for (int j = 0; j < C.n; ++j) {
                        if (!C.arr[i][j])
                            throw std::runtime_error(":operator/:1: division by zero");
                        C.arr[i][j] = a / C.arr[i][j];
                    }
                return C;
            }
    ...
    namespace MX {
        ...
        : m(r), n(c), arr(nullptr) {
            if (m <= 0 || n <= 0)
                throw std::runtime_error(":Matrix:1: incorrect dimensions");
            Create();
        }
        ...
        : m(r), n(c), arr(nullptr) {
            if (m <= 0 || n <= 0)
                throw std::runtime_error(":Matrix:2: incorrect dimensions");
            if (!a)
                throw std::runtime_error(":Matrix:2: can't copy from a null array");
            Create(a);
        }
        ...
        : m(r), n(c), arr(nullptr) {
            if (m <= 0 || n <= 0)
                throw std::runtime_error(":Matrix:3: incorrect dimensions");
            if (!a)
                throw std::runtime_error(":Matrix:3: can't copy from a null array");
            Create(a);
        }
        ...
        : m(l.size()), n(l.begin()->size()), arr(nullptr) {
            ...
            for (int i = 0; i < m; ++i) {
                if ((l.begin()+i)->size() != n)
                    throw std::runtime_error(":Matrix:4: incorrect initializer list");
                for (int j = 0; j < n; ++j)
        ...
        Matrix<T>& Matrix<T>::operator= (std::initializer_list<std::initializer_list<T>> l) {
            ...
            for (int i = 0; i < m; ++i) {
                if ((l.begin()+i)->size() != n)
                    throw std::runtime_error(":operator=:1: incorrect initializer list");
                for (int j = 0; j < n; ++j)
                    arr[i][j] = *((l.begin()+i)->begin()+j);
            }
            return *this;
        ...
        Matrix<T>& Matrix<T>::fit (const Matrix& B) {
            if (!arr)
                throw std::runtime_error(":fit:1: matrix not initialized");
            if (B.m * B.n != m * n)
                throw std::runtime_error(":fit:1: sizes not match");
            if (B.m == m && B.n == n) {
                for (int i = 0; i < m; ++i)
                    for (int j = 0; j < n; ++j)
                        arr[i][j] = B.arr[i][j];
        ...
        Matrix<T>& Matrix<T>::randomize (float a, float b) {
            if (!arr)
                throw std::runtime_error(":randomize:1: matrix not initialized");
                float diff =  b - a;
            for (int i = 0; i < m; ++i)
                for (int j = 0; j < n; ++j)
        ...
        Matrix<T> Matrix<T>::flatten (int axis) const {
            if (!arr)
                throw std::runtime_error(":flatten:1: matrix not initialized");
        ...
        Matrix<T> Matrix<T>::reshape (int r, int c) const {
            if (!arr)
                throw std::runtime_error(":reshape:1: matrix not initialized");
            if (r <= 0 || c <= 0)
                throw std::runtime_error(":reshape:1: incorrect dimensions");
            if (m * n != r * c)
                throw std::runtime_error(":reshape:1: sizes not match");
            if (r == m && c == n) 
                return *this;
            Matrix C(r, c);
            int Ci = 0, Cj = 0;
        ...
        Matrix<T> Matrix<T>::transpose () const {
            if (!arr)
                throw std::runtime_error(":transpose:1: matrix not initialized");
            Matrix C(n, m);
            for (int i = 0; i < m; ++i)
                for (int j = 0; j < n; ++j)
                    C.arr[j][i] = arr[i][j];
        ...
        Matrix<T> Matrix<T>::rotate180 () const {
            if (!arr)
                throw std::runtime_error(":rotate180:1: matrix not initialized");
            Matrix C(m, n);
            for (int i = 0; i < m; ++i)
                for (int j = 0; j < n; ++j)
        ...
        Matrix<T> Matrix<T>::apply (T (*func) (T)) const {
            if (!arr)
                throw std::runtime_error(":apply:1: matrix not initialized");
            if (!func)
                throw std::runtime_error(":apply:1: cannot apply null function");
            Matrix C = *this;
            for (int i = 0; i < m; ++i)
                for (int j = 0; j < n; ++j)
        ...
        Matrix<T> Matrix<T>::apply (T (*func) (T, int, float), int mode, float hp) const {
            if (!arr)
                throw std::runtime_error(":apply:2: matrix not initialized");
            if (!func)
                throw std::runtime_error(":apply:2: cannot apply null function");
            Matrix C = *this;
            for (int i = 0; i < m; ++i)
                for (int j = 0; j < n; ++j)
        ...
        T& Matrix<T>::operator() (int i, int j) {
            if (!arr)
                throw std::runtime_error(":operator():1: matrix not initialized");
            if (i < 0 || j < 0)
                throw std::runtime_error(":operator():1: negative index");
            if (i >= m || j >= n)
                throw std::runtime_error(":operator():1: index out of bounds");
            return arr[i][j];
        }
        ...
        T const& Matrix<T>::operator() (int i, int j) const {
            if (!arr)
                throw std::runtime_error(":operator():2: matrix not initialized");
            if (i < 0 || j < 0)
                throw std::runtime_error(":operator():2: negative index");
            if (i >= m || j >= n)
                throw std::runtime_error(":operator():2: index out of bounds");
            return arr[i][j];
        }
        ...
        Matrix<T> Matrix<T>::operator/ (const T b) const {
            if (!arr)
                throw std::runtime_error(":operator/:2: matrix not initialized");
            if (!b)
                throw std::runtime_error(":operator/:2: division by zero");
            Matrix C = *this;
            for (int i = 0; i < m; ++i)
                for (int j = 0; j < n; ++j)
                    C.arr[i][j] /= b;
        ...
        Matrix<T> Matrix<T>::operator+ (const Matrix& B) const {
            if (!arr || !B.arr)
                throw std::runtime_error(":operator+:3: matrix not initialized");
            if (m != B.m || n != B.n)
                throw std::runtime_error(":operator+:3: dimentions not match");
            Matrix C = *this;
            for (int i = 0; i < m; ++i)
                for (int j = 0; j < n; ++j)
                    C.arr[i][j] += B.arr[i][j];
        ...
        Matrix<T> Matrix<T>::operator/ (const Matrix& B) const {
            ...
            for (int i = 0; i < m; ++i) 
                for (int j = 0; j < n; ++j) {
                    if (!B.arr[i][j])
                        throw std::runtime_error(":operator/:3: division by zero"); 
                    C.arr[i][j] /= B.arr[i][j];
                }
            return C;
        }
        ...
        Matrix<T> Matrix<T>::operator- () const {
            if (!arr)
                throw std::runtime_error(":operator-:4: matrix not initialized");
            return (*this) * -1;
        }
        ...
        Matrix<T>& Matrix<T>::operator+= (const Matrix& B) {
            if (!arr || !B.arr)
                throw std::runtime_error(":operator+=:2: matrix not initialized");
            if (m != B.m || n != B.n)
                throw std::runtime_error(":operator+=:2: dimentions not match");
            for (int i = 0; i < m; ++i)
                for (int j = 0; j < n; ++j)
                    arr[i][j] += B.arr[i][j];
            return *this;
        ...
        void Matrix<T>::Create () {
            if (m <= 0 || n <= 0)
                throw std::runtime_error(":Create:1: incorrect dimensions");
            arr = new T*[m];
            for (int i = 0; i < m; ++i)
                arr[i] = new T[n] {0};
        }
        ...
        void Matrix<T>::Create (T* a) {
            if (m <= 0 || n <= 0)
                throw std::runtime_error(":Create:2: incorrect dimensions");
            if (!a)
                throw std::runtime_error(":Create:2: can't copy from a null array");
            arr = new T*[m];
            for (int i = 0, index = 0; i < m; ++i) {
                arr[i] = new T[n];
                for (int j = 0; j < n; ++j, ++index)
        ...
        void Matrix<T>::Create (const T* a) {
            if (m <= 0 || n <= 0)
                throw std::runtime_error(":Create:3: incorrect dimensions");
            if (!a)
                throw std::runtime_error(":Create:3: can't copy from a null array");
            arr = new T*[m];
            for (int i = 0, index = 0; i < m; ++i) {
                arr[i] = new T[n];
                for (int j = 0; j < n; ++j, ++index)
        ...
        void Matrix<T>::Create (T** a) {
            if (m <= 0 || n <= 0)
                throw std::runtime_error(":Create:4: incorrect dimensions");
            if (!a)
                throw std::runtime_error(":Create:4: can't copy from a null array");
            arr = new T*[m];
            for (int i = 0; i < m; ++i) {
                arr[i] = new T[n];
                for (int j = 0; j < n; ++j)
        ...
        void Matrix<T>::Create (const T** a) {
            if (m <= 0 || n <= 0)
                throw std::runtime_error(":Create:5: incorrect dimensions");
            if (!a)
                throw std::runtime_error(":Create:5: can't copy from a null array");
            arr = new T*[m];
            for (int i = 0; i < m; ++i) {
                arr[i] = new T[n];
                for (int j = 0; j < n; ++j)
    ...
    namespace MX {
        ...
        std::istream& operator>> (std::istream& is, Matrix<T>& A) {
            ...
            while (braces > 0) {
                if (is.peek() == EOF)
                    throw std::runtime_error(":operator>>:1: stream is incomplete");
                switch (is.peek()) {
                    case '{':
                        if (braces > 1)
                            throw std::runtime_error(":operator>>:1: too many opening braces");
                    ...
                    default:
                        if (braces != 2)
                            throw std::runtime_error(":operator>>:1: brace is missing");
                        if ((is.peek() < 48 || is.peek() > 57) && is.peek() != '-')
                            throw std::runtime_error(":operator>>:1: unknown symbol");
                        T number;
                        is >> number;
            ...
            for (int i = 0; i < A.m; ++i) {
                if (t[i].size() != A.n)
                    throw std::runtime_error(":operator>>:1: incorrect list dimensions");
                for (int j = 0; j < A.n; ++j)
                    A.arr[i][j] = t[i][j]; 
            }
        ...
        Matrix<T> Dot (const Matrix<T>& A, const Matrix<T>& B) {
            if (!A.arr || !B.arr)
                throw std::runtime_error(":Dot:1: matrix not initialized");
            if (A.n != B.m)
                throw std::runtime_error(":Dot:1: multiplication conditions not met");
            Matrix<T> C(A.m, B.n);
            for (int i = 0; i < A.m; ++i)
                for (int j = 0; j < B.n; ++j)
                    for (int k = 0; k < A.n; ++k)
        ...
        T Sum (const Matrix<T>& A) {
            if (!A.arr)
                throw std::runtime_error(":Sum:1: matrix not initialized");
            T sum = A.arr[0][0];
            for (int i = 0; i < A.m; ++i)
                for (int j = 0; j < A.n; ++j)
                    sum += A.arr[i][j];
        ...
        Matrix<T> Sum (const Matrix<T>& A, int axis) {
            if (!A.arr)
                throw std::runtime_error(":Sum:2: matrix not initialized");
            Matrix<T> sum;
            if (axis == 0) {
                sum = Matrix<T>(1, A.n);
                for (int i = 0; i < A.m; ++i)
        ...
        Matrix<T> Convolve (const Matrix<T>& A, const Matrix<T>& kernel, int padding, int stride) {
            if (!A.arr || !kernel.arr)
                throw std::runtime_error(":Convolve:1: matrix not initialized");
            if (stride < 1)
                throw std::runtime_error(":Convolve:1: stride is less than one");
            int cm = (A.m + 2*padding - kernel.m) / stride + 1;
            int cn = (A.n + 2*padding - kernel.n) / stride + 1;
            if (cm <= 0 || cn <= 0)
                throw std::runtime_error(":Convolve:1: kernel too large");
            Matrix<T> C(cm, cn);
        ...
        std::istream& operator>> (std::istream& is, std::vector<std::vector<Matrix<T>>>& A) {
            ...
            while (braces > 0) {
                if (is.peek() == EOF)
                    throw std::runtime_error(":operator>>:2: stream is incomplete");
                switch (is.peek()) {
                    case '{':
                        if (braces > 2)
                            throw std::runtime_error(":operator>>:2: too many opening braces");
                        if (braces == 1) {
                    ...
                    case '}':
                        ...
                        is.ignore(); break;
                    default:
                        throw std::runtime_error(":operator>>:2: unknown symbol");
                }
            }
            return is;
        }
